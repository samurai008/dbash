...
.controller('AppCtrl', function($scope, $ionicModal, $timeout, $http, $localStorage, $state, $ionicTabsDelegate, $state) {
...
.controller('EditProfileCtrl', function($scope, $http, $location, $ionicHistory, $document, $ionicGesture, $state, $base64) {
  ...
  $scope.editProfile = function () {
    ...
    $http({
      method: 'PUT',
      url: 'http://localhost:8000/user/' + $scope.$userData.username + '/',
...
.controller('EventListCtrl', function($scope, $http) {
  ...
  $http({
    method: 'GET',
    url: 'http://localhost:8000/events/?q=',
  }).then(function successCallback(response){
    console.log(response.data);
  ...
  $scope.search = function() {
    ...
    $http({
      method: 'GET',
      url: 'http://localhost:8000/events/?q='+$scope.str.query,
    }).then(function successCallback(response) {
      $scope.insearch = false;
      console.log(response.data);
      $scope.events = response.data;
...
.controller('ClubListCtrl', function($scope, $http) {
  ...
  $http({
    method: 'GET',
    url: 'http://localhost:8000/clubs/?q=',
  }).then(function successCallback(response){
    console.log(response.data);
  ...
  $scope.search = function() {
    ...
    $http({
      method: 'GET',
      url: 'http://localhost:8000/clubs/?q='+$scope.str.query,
    }).then(function successCallback(response) {
      $scope.insearch = false;
      console.log(response.data);
      $scope.clubs = response.data;
...
.controller('ClubDetailCtrl', function(
  ...
  $http({
    method: 'GET',
    url: 'http://localhost:8000/club/'+$stateParams.id,
  }).then(function successCallback(response){
    $scope.hide();
    console.log(response.data);
...
.controller('EventDetailCtrl', function($scope, $stateParams, $http, $ionicLoading) {
  ...
  $http({
    method: 'GET',
    url: 'http://localhost:8000/event/'+$stateParams.id,
  }).then(function successCallback(response){
    $scope.hide();
    console.log(response.data);
...
.controller('FindClubsDetailCtrl', function($scope, $cordovaGeolocation, $http, $ionicLoading) {
  ...
  $scope.getclubs = function(lt, lg) {
    $http({
      method: 'GET',
      url: 'http://localhost:8000/searchnearclubs/?lat='+ lt +'&lon=' + lg,
    }).then(function successCallback(response) {
      $scope.hide();
      console.log(response);
      $scope.clubs = response.data;
